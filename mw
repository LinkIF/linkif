// ==UserScript==
// @name         MW v1.3_redesign_final (panelfix for moswar redesign)
// @author       barifan + LinkIF + Copilot
// @namespace    Linkif
// @version      1.4.1
// @description  Отправка/заправка/комплекты/статус/очистка/слоты/СТАРЫЙ ДИЗАЙН, ПОЛНАЯ РАБОТОСПОСОБНОСТЬ с новым сайтом
// @include        *moswar.ru*
...
console.log("=== Проверка userscript ===", window.location.pathname);
...
`,
    ...
    async function Me() {
      ...
      // ============= КОД ПАНЕЛИ КОМПЛЕКТОВ =============

function renderKitPanel() {
    let target = document.querySelector("#content > div > div.block-bordered")
             || document.querySelector("#garage .garageform")
             || document.body;
    if (!target) return;
    target.insertAdjacentHTML('afterbegin', '<div style="background:purple;color:#fff;padding:10px;">PANEL TEST</div>');
}
    ...
    if (location.pathname.startsWith("/automobile/ride")) {
        const garageReady = setInterval(() => {
            const block = document.querySelector("#content > div > div.block-bordered")
                        || document.querySelector("#garage .garageform");
            if (block) {
                console.log("=== Блок гаража найден, вставляю панель ===");
                renderKitPanel();
                clearInterval(garageReady);
            } else {
                console.log("Пробую найти блок для панели...");
            }
        }, 300);
    }
}
...
// Если хочешь, чтобы панель не исчезала при ajax/SPA — раскомментируй эти строки:
// const observer = new MutationObserver(renderKitPanel);
// observer.observe(document.body, { childList: true, subtree: true });

// ==== И вот теперь — САМАЯ последняя скобка! ====
