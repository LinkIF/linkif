// ==UserScript==
// @name         MW v1.3_redesign_final
// @author       barifan
// @namespace    Linkif
// @version      1.4
// @description  Реализовано: отправка/заправка/комплекты/статус/очистка/слоты/стабильная панель
// @include      *moswar.ru*
...
console.log("=== MW userscript loaded ===");

(async function () {

// ... (оставь все твои utils_, функции боёв, петарены и т.д. — ничего не трогай из них!)
// ... (ниже только код для панели комплектов!)



/**
 * Главная функция создания панели комплектов.
 * Автоматически вставляется в нужный контейнер гаража.
 * Все обработчики и отображение — внутри.
 */
function renderKitPanel() {
    // Удаляем старую панель, если она есть
    document.getElementById('moswar-kit-panel')?.remove();

    // Вёрстка панели комплектов
    const html = `
        <div id="moswar-kit-panel" style="background:#fff8e1;border:2px solid #ffcc80;padding:10px 15px;border-radius:12px;box-shadow:0 4px 8px rgba(0,0,0,0.1);width:fit-content;margin:10px;font-family:Arial,sans-serif;">
            <div id="kit-slot-list" style="margin-bottom:8px;"></div>
            <div class="mw-status-line" id="mw-status">Ожидание действий...</div>
            <div style="margin-top: 10px;">
                <button class="mw-btn" id="btn-save">💾 Сохранить</button>
                <button class="mw-btn" id="btn-send-current">🟢 Отправить выбранное</button>
                <button class="mw-btn" id="btn-refuel">⛽ Заправить всё</button>
                <select id="kit-select" class="mw-select" style="margin-left:7px;margin-right:7px;"></select>
                <button class="mw-btn" id="btn-send-kit">📤 Отправить комплект</button>
            </div>
        </div>
    `;

    // Вставка панели в нужный контейнер гаража (или в body, если не найден)
    let target = document.querySelector("#garage .garageform") || document.body;
    ...
    // --- Обработчики кнопок ---
    document.getElementById("btn-save").onclick = saveKit;
    document.getElementById("btn-send-current").onclick = sendSelectedNow;
    document.getElementById("btn-refuel").onclick = refuelAll;
    document.getElementById("btn-send-kit").onclick = sendKitFromSelect;
}

/**
 * Получение комплектов из localStorage (ключ moswar_saved_sets)
 */
function getKits() {
    return JSON.parse(localStorage.getItem("moswar_saved_sets") || "{}");
}

/**
 * Сохранение комплектов в localStorage
 */
function setKits(kits) {
    localStorage.setItem("moswar_saved_sets", JSON.stringify(kits));
}

/**
 * Обновление интерфейса слотов (отображение 4-х слотов и “крестиков” сброса)
 */
function updateKitSlotList() {
    const kits = getKits();
    const el = document.getElementById("kit-slot-list");
    el.innerHTML = '';
    for(let i=1; i<=4; i++) {
        let count = Array.isArray(kits[i]) ? kits[i].length : 0;
        el.innerHTML += `
            <span style="display:inline-block;margin:4px;padding:6px 10px 6px 30px;border:1px solid #ffb74d;border-radius:6px;background:#ffe0b2;font-weight:bold;position:relative;min-width:80px;">
                <button style="position:absolute;top:-6px;left:-6px;background:#ef5350;color:white;border:none;border-radius:50%;width:18px;height:18px;font-size:12px;cursor:pointer;"
                    onclick="resetSlotKit(${i})">×</button>
                Слот ${i} (${count})
            </span>
        `;
    }
...
/**
 * Глобальная функция для onClick сброса слота по “крестику”
 */
window.resetSlotKit = function(idx) {
    let kits = getKits();
    delete kits[idx];
    setKits(kits);
    updateKitSlotList();
    updateKitSelect();
    updateKitStatus('Слот ' + idx + ' очищен');
};

/**
 * Обновление выпадающего списка комплектов
 */
function updateKitSelect() {
    ...
    for(let i=1; i<=4; i++) {
        let count = Array.isArray(kits[i]) ? kits[i].length : 0;
        select.innerHTML += `<option value="${i}">Слот ${i} (${count})</option>`;
    }
}

/**
 * Обновление статус-бара (сообщения пользователю)
 */
function updateKitStatus(text) {
    if(text) document.getElementById("mw-status").textContent = text;
}

/**
 * Сохранение текущего набора выбранной техники в указанный слот
 */
function saveKit() {
    // Собираем выбранные машины (через It, как в твоём коде)
    const rides = It();
    if (!rides.length) return updateKitStatus("Нет выбранной техники");
    const slot = prompt("В какой слот сохранить? (1-4)");
    if (!['1','2','3','4'].includes(slot)) return updateKitStatus("Неверный слот");
    let kits = getKits();
    kits[slot] = rides;
    setKits(kits);
    updateKitSlotList();
    updateKitSelect();
    updateKitStatus("Сохранено в слот " + slot);
}

/**
 * Отправить все выбранные в данный момент машины (чекбоксы)
 */
function sendSelectedNow() {
    const rides = It();
    if(!rides.length) return updateKitStatus("Выберите машины");
    sendRidesArray(rides, "Отправлено", rides.length);
}

/**
 * Отправить комплект из выбранного в списке слота
 */
function sendKitFromSelect() {
    const slot = document.getElementById("kit-select").value;
    let kits = getKits();
    const rides = kits[slot];
    if(!rides || !rides.length) return updateKitStatus("Комплект не найден");
    sendRidesArray(rides, `Отправлено из слота ${slot}`, rides.length);
}

/**
 * Асинхронно отправить (и заправить, если нужно) массив машин
 * Статус: "Отправлено X/Y", "Заправлено X/Y"
 */
async function sendRidesArray(rides, msg, total) {
    let refueled = 0, launched = 0, refuelTries = 0;
    for(let {carId,rideId} of rides) {
        try {
            // Проверка топлива (если нет — заправляем)
            const res = await fetch(`/automobile/car/${carId}/`, { credentials: "include" }).then(r=>r.text());
            const fuelMatch = res.match(/Топливо:[^0-9]*([0-9]+)/i);
            ...
            launched++;
            updateKitStatus(`${msg} ${launched}/${total}, Заправлено ${refueled}/${refuelTries}`);
        } catch (e) {
            updateKitStatus(`Ошибка отправки/заправки машины ${carId}`);
        }
    }
    updateKitStatus(`${msg} ${launched}/${total}, Заправлено ${refueled}/${refuelTries}`);
}
...
/**
 * Заправить все машины в гараже
 * Статус: "Заправлено X/Y"
 */
async function refuelAll() {
    const cars = Array.from(document.querySelectorAll('input[name="car"]'));
    let refueled = 0, tries = 0;
    for (const input of cars) {
        tries++;
        try {
            await fetch(`/automobile/buypetrol/${input.value}/`, {
                method: "POST", credentials: "include",
                headers: { "Content-Type": "application/x-www-form-urlencoded" },
                body: `__ajax=1&return_url=%2Fautomobile%2Fcar%2F${input.value}`
            });
            refueled++;
            updateKitStatus(`Заправлено ${refueled}/${tries}`);
        } catch(e){
            updateKitStatus(`Ошибка заправки ${input.value}`);
        }
    }
    updateKitStatus(`Заправлено ${refueled}/${tries}`);
}
...
/**
 * Вспомогательная функция: собрать выбранные машины (оставить как есть!)
 * (It() — уже есть у тебя, не трогаем)
 */


/*
 * Вставляем панель комплектов при загрузке гаража (страница /automobile/ride/)
 */
function tryRenderKitPanel() {
    if (location.pathname.startsWith("/automobile/ride")) {
        const garageReady = setInterval(() => {
            const block = document.querySelector("#content > div > div.block-bordered");
            if (carsTrip) {
                // Вставка панели
                carsTrip.insertAdjacentHTML('afterbegin', '<div>Твоя панель комплектов тут</div>');
                clearInterval(garageReady);
            }
        }, 300);
    }
}
document.addEventListener("DOMContentLoaded", tryRenderKitPanel);

document.addEventListener("DOMContentLoaded", tryRenderKitPanel);

// При загрузке страницы и при ajax-навигации — всегда отрисовываем панель, если нужная страница
document.addEventListener("DOMContentLoaded", tryRenderKitPanel);

// Если у тебя есть ajax-переходы — можно добавить MutationObserver или слушатель на события смены url
// Но для большинства случаев хватит setInterval выше



/* --- ОСТАЛЬНОЙ КОД (utils_, функции боёв, петарены, другие интерфейсы) — не трогай! --- */

// ... (оставь твои utils_, функции боёв, петарены и т.д.)
...
async function In() {
  ...
  if (t.match(/\/alley\/fight\//)) {
    ...
    let n = k({
        text: "\u{1F695}",
        title:
          "\u0411\u043E\u043C\u0431\u0438\u0442\u044C \u0431\u0435\u0441\u043F\u043B\u0430\u0442\u043D\u043E (\u043F\u043E\u043A\u0430 \u0432\u043A\u043B\u0430\u0434\u043A\u0430 \u043D\u0435 \u0437\u0430\u043A\u0440\u044B\u0442\u0430)",
  ...
  "><img src="/@/images/link/taxi.jpg" style="width: 48px;"> <span>\u0420\u0435\u0436\u0438\u043C \u0431\u043E\u043C\u0431\u0438\u0442\u044C \u0431\u0435\u0441\u043F\u043B\u0430\u0442\u043D\u043e.<br> `
            );
        },
      }),
