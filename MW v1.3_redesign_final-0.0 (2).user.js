...
(async function () {
			 ...
			 // ========== ЛОГИКА: сохранение, отправка и индикатор комплекта_начало ==========
...
// === Исправленная вставка панели только в нужный контейнер ===
document.addEventListener("DOMContentLoaded", () => {
  const observer = new MutationObserver(() => {
    const target = document.querySelector("#garage .garageform");
    if (typeof Ut === "function" && target && !target.querySelector("#moswar-kit-panel")) {
      console.log("[🧪] Вставка панели Ut()");
      Ut();
      observer.disconnect();
    }
  });
